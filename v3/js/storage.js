...
export class StorageService {
    ...
    _loadLogs() {
        try {
            ...
            // Fallback to empty array (in-memory only)
            return [];
        }
    }
    ...
    /**
     * Generates a sensitivity report identifying trigger and safe foods.
     * @returns {Object} { triggers: Array, safe: Array }
     */
    ...
    /**
     * Aggregates nutrient data by mood.
     * @returns {Object} { moodData: { "Good": { calories: 0, protein: 0, count: 0 }, ... } }
     */
    getNutrientTrends() {
        const moodData = {};
        ...
        this.logs.forEach(log => {
            ...
            // Parse macros (remove 'g' and parse int)
    ...
    getStreak() {
        ...
        const dates = [...new Set(this.logs.map(log => new Date(log.timestamp).toDateString()))];
        ...
        if (dates[0] === today) {
            ...
            streak = 1; // Streak is still alive if logged yesterday
        } else {
